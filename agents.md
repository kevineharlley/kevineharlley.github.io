...
**Theme colors**: the palette (`emerald`, `gold`, `amethyst`) is defined once as CSS custom properties in the `@theme` block of [globals.css](src/app/globals.css) (`--color-emerald`, `--color-gold`, `--color-amethyst`). Tailwind v4 auto-generates utility classes from these (`text-emerald`, `bg-gold/10`, `shadow-emerald/25`, etc. — opacity modifiers work on any of them).

- Always reach for the generated Tailwind utility classes, or the shared maps in [src/lib/accent.ts](src/lib/accent.ts) when a color needs to be selected dynamically from a component prop.
